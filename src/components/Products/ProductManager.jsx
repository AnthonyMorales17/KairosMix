...
const ProductManager = () => {
  ...
  // Cargar productos desde Dexie IndexedDB
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: 'No se pudieron cargar los productos de la base de datos',
        });
      }
    };
  ...
  const performSearch = async (searchQuery) => {
    ...
    try {
      ...
      // 4.1: Exact code search (usando Dexie)
      const exactCodeMatch = await db.products
        .where('code')
      ...
      // 4.2 & 4.3: Partial name search (búsqueda en todos los productos)
      const allProducts = await db.products.toArray();
  ...
  const handleAddProduct = async (productData) => {
    try {
      ...
      // Agregar a Dexie (auto-genera el ID)
